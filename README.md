...
**기술적 도전과 해결 경험**
* **법률 검색 정확도 향상을 위한 임베딩 모델 파인튜닝**: 일반 임베딩 모델은 법률 용어의 미묘한 의미 차이를 구분하지 못해, 사용자의 질문 의도와 다른 판례를 검색하는 문제가 있었습니다. RAG 시스템의 신뢰성은 정확한 문서 검색에 달려있기에, 법률 도메인에 특화된 임베딩 모델 개발을 목표로 삼았습니다. 고성능의 사전 학습된 임베딩 모델을 선정하고, 직접 구축한 국내 판례 데이터셋으로 미세조정(Fine-tuning)을 진행했습니다. 이 과정을 통해 개발된 특화 모델은 RAG 파이프라인의 '검색' 단계에서 유사 판례 검색 정확도를 크게 향상시켰습니다. 생성 모델에 더 정확한 컨텍스트를 제공함으로써, AI 답변이 실제 판례에 근거하도록 만들어 서비스의 핵심 신뢰도를 확보할 수 있었습니다.
