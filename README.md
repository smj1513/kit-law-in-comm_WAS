...
> 법률 상담 도메인에 특화된 답변 생성을 위해 Ko-LLM 임베딩 모델을 선정하고 Fine-tuning을 통해 성능을 최적화했습니다.

> 백엔드 시스템과 AI 모델 서버 간의 안정적인 비동기 통신 방식을 설계하여 시스템 안정성을 확보했습니다.
...
| **👨‍⚖️ 전문가 1:1 상담** | **전문가 매칭 시스템** | 민사, 형사, 가사 등 13개의 세분화된 법률 분야에 맞춰 최적의 변호사를 추천하고 연결합니다. |
| | **실시간 채팅 상담** | WebSocket(STOMP) 기반의 1:1 채팅 기능을 통해 사용자와 변호사 간의 원활하고 즉각적인 소통을 지원합니다. |
...
1.  **실시간 채팅의 안정성 확보**: 대규모 동시 접속 상황을 가정하여 WebSocket의 세션 관리와 메시지 브로커(STOMP)의 동작 원리를 깊이 있게 학습했습니다. 또한, 채팅 메시지와 알림의 데이터 정합성을 보장하기 위해 `@Transactional`의 전파 옵션을 신중하게 적용했습니다.
2.  **데이터 무결성 강화**: DB 레벨의 `UNIQUE 제약조건`과 `@Pattern` 같은 Validation 어노테이션을 적극 활용하여 잘못된 데이터의 입력을 원천적으로 차단했습니다. 또한, `orphanRemoval=true`, `@OnDelete` 옵션을 통해 연관된 데이터가 일관성 있게 관리되도록 설계했습니다.
3.  **CI/CD 파이프라인 구축**: `GitHub Actions`를 사용하여 main 브랜치에 코드가 병합될 때마다 자동으로 빌드, 테스트, Docker 이미지 빌드 및 배포가 이루어지는 파이프라인을 구축했습니다. 이를 통해 수동 배포의 실수를 줄이고 개발 생산성을 크게 향상시켰습니다.
...
* **협업 및 커뮤니케이션**: Git-flow 브랜치 전략을 기반으로 협업했으며, Swagger를 통한 명확한 API 문서화로 프론트엔드 팀과의 원활한 소통을 이끌었습니다.
